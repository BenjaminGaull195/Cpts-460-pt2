...
int body()   // process body function
{
  ...
  */
  ...
  timerenqueue(&timerQueue, q);

  ksleep(running);
}

int addchild(PROC *parent, PROC *child) {
  if (parent->child == 0) {
    parent->child = child;
    return 0;
  }
  else {
    return addsibling(parent->child, child);
  }
}

int addsibling(PROC *current, PROC *newSibling) {
  if (current->sibling == 0) {
    current->sibling = newSibling;
    return 0;
  }
  else {
    return addsibling(current->sibling, newSibling);
  }
}
